...
public class randomItemPicker extends PApplet {
...
public void draw() {
  ...
  if (itemIndex >= items.length) {
    ...
    if (pic.height > height - 120) {
      float scaleFactor = PApplet.parseFloat(height - 120) / pic.height;
      pic.resize(PApplet.parseInt(pic.width * scaleFactor), height - 120 );
    }
...
public String getFlickrPic(String searchString) {
    ...
    int r = PApplet.parseInt(random(10));
...
public void shuffleItems() {
  for (int i = 0; i < items.length; i++) {
    int newPos = PApplet.parseInt(random(items.length));
    String temp = items[i];
...
public boolean sketchFullScreen() {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "randomItemPicker" };
    if (passedArgs != null) {
